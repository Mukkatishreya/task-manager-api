...
taskmanagerapi/ ├── app.py ├── routes.py ├── models.py ├── extensions.py ├── requirements.txt

⚙️ How the Project Works (High-Level Flow)
Client (Postman / frontend application) sends an HTTP request
Flask application receives and routes the request
Business logic processes the request
SQLAlchemy ORM interacts with the SQLite database
